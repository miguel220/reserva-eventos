...
@section('content')
<div class="p-6">
    <h1 class="text-3xl font-semibold mb-4">Editar Usuário: {{ $user->name }}</h1>
    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror" required>
            @error('name')
        ...
        <div class="mb-4">
            ...
            <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('email') border-red-500 @enderror" required>
            @error('email')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $user->phone_number) }}" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('phone_number') border-red-500 @enderror">
            @error('phone_number')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="password" name="password" id="password" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('password') border-red-500 @enderror">
            @error('password')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="password" name="password_confirmation" id="password_confirmation" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="is_producer" id="is_producer" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('is_producer') border-red-500 @enderror" required>
                <option value="0" {{ old('is_producer', $user->is_producer) == 0 ? 'selected' : '' }}>Não</option>
                <option value="1" {{ old('is_producer', $user->is_producer) == 1 ? 'selected' : '' }}>Sim</option>
            </select>
            @error('is_producer')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        ...
        <div class="mb-4">
            ...
            <select name="setor_id" id="setor_id" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('setor_id') border-red-500 @enderror">
                ...
                @foreach ($setores as $setor)
                    <option value="{{ $setor->id }}" {{ old('setor_id', $user->setor_id) == $setor->id ? 'selected' : '' }}>{{ $setor->nome }}</option>
                @endforeach
            </select>
            @error('setor_id')
                <span class="text-red-500 text-sm">{{ $message }}</span>
