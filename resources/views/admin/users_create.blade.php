...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-semibold">Adicionar Usuário</h1>
        ...
        <form method="POST" action="{{ route('admin.users.store') }}">
            ...
            <div class="mb-6">
                ...
                <select 
                    ...
                    required 
                >
                    <option value="0" {{ old('is_admin', 0) == 0 ? 'selected' : '' }}>Não</option>
                    <option value="1" {{ old('is_admin', 0) == 1 ? 'selected' : '' }}>Sim</option>
                </select>
                @error('is_admin')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
